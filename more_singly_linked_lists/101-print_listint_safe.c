...
/**
 * create_addr_node - creates a new node storing an address
 * @addr: address to store
 *
 * Return: pointer to new node, exits with 98 if malloc fails
 */
static addr_node_t *create_addr_node(const listint_t *addr)
{
...
size_t print_listint_safe(const listint_t *head)
{
	...
	while (head != NULL)
	{
		...
		new_node = create_addr_node(head);
		if (visited == NULL)
		{
			visited = new_node;
			tail = new_node;
